...
class AdmiPushCNNEE(Basetask):
    def __init__(self, cfg):
        ...
        self.prev_ee_pos = th.zeros((1,3), device = "cpu")
    ...
    def reset(self, env_ids: Sequence[int] | None):
       ...
       # # particle position 
        ...
        self.prev_ee_pos = ee_pose[:,0:3].clone().to(self.device)
    ...
    def apply_action(self):
        ...
        delta_ee = ee_pose[:,0:3] - self.prev_ee_pos
        self.prev_ee_pos = ee_pose[:,0:3].clone().to(self.device)
        ee_diff_vel = self.estimate_obj_velocity(self.sim_dt, delta_ee)

        self.admittance_controller.set_delta_pos(delta_ee)
        self.admittance_controller.set_delta_vel(ee_diff_vel)

        self.logging_data = {
                "episode": self.episode_cpt,
                "brush_position": ee_pose[:,0:3].squeeze(dim=0).cpu().numpy().tolist(),
                "debris_position": self.part_pos.squeeze(dim=0).cpu().numpy().tolist(),
        ...
        pos_command_xy = ee_pose[:,0:2] - pos_command_[:, 0:2]
