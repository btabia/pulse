...
class AdmiPushCNN(Basetask):
    ...
    def get_observations(self):
        ...
        self.ee_position = ee_pose[:,0:2].to(self.device)
       ...
       # out = th.stack((hmap, tmap))
       # self.write_image_to_container(out.to("cpu").numpy())
    ...
    def object_out_of_bound(self): 
        obj_pos, _ = self.obj_prim.get_world_poses()
        self.bad_episode = False
        if obj_pos[:,[0]] < self.cfg["task"]["vision"]["min_x"]:
            self.bad_episode = True
        if obj_pos[:,[0]] > self.cfg["task"]["vision"]["max_x"]:
            self.bad_episode = True
        if obj_pos[:,[1]] < self.cfg["task"]["vision"]["min_y"]:
            self.bad_episode = True
        if obj_pos[:,[1]] > self.cfg["task"]["vision"]["max_y"]:
            self.bad_episode = True
        if self.bad_episode == True: 
            print("Object out of bound")

    def is_done(self) -> tuple[th.Tensor, th.Tensor]:
        self.object_out_of_bound()
        done = False
    ...
    def process_actions(self, actions): 
        ...
        self.admittance_controller.set_force_command(F_command)

    def update_obj_pos(self):
        #self._obj_pos = th.zeros((1,3), device = self.device)
        #self._obj_pos[:,0:2] = self.part_pos
        ...
        #print("simulation freq : " + str(1/self.sim_dt))
    ...
    def estimate_obj_velocity(self, dt, obj_pos):
        _dt = th.tensor([[dt, dt, dt]], device = self.device)
    ...
    def apply_action(self):
        ...
        #medium = self.dvision.process_point_cloud(self.cameras, self.sim.get_physics_dt())
        #if th.numel(medium) == 0:
        #    print("zthere is no points in the point cloud")
        #    self.bad_episode = True
        
        # cartesian and absolutes distances
        #ee_obj_pos, __, ___, ____, obj_pos = self.dvision.calculate_distances(
        #    medium_tensor_pc = medium, 
        #    target_position=self.target_position,
        #    ee_position = self.ee_position, 
        #    medium_visualiser = self.particule_middle_vis
        ...
        ee_efforts_xyz = ee_efforts_xyz[0:3]
        ee_efforts_xyz = ee_efforts_xyz.unsqueeze(dim = 0)

        self.admittance_controller.set_force_observation(ee_efforts_xyz)

        #_obj_pos, _ = self.obj_prim.get_world_poses()

        
        ee_pose = self.robot.data.body_state_w[:, self.robot_entity_cfg.body_ids[0], 0:7].clone().to(self.device)
        #ee_obj_pos = th.tensor([[self._obj_pos[0][0], self._obj_pos[0][1], self.cfg["task"]["action"]["z_command"]]], device = self.device)
        ee_obj_pos = self._obj_pos.to(self.device) - ee_pose[:,0:3]
        ee_particle_vel = self.estimate_obj_velocity(self.sim_dt, ee_obj_pos)

        self.admittance_controller.set_delta_pos(ee_obj_pos)
        self.admittance_controller.set_delta_vel(ee_particle_vel)

        self.logging_data = {
                "episode": self.episode_cpt,
                "brush_position": ee_pose[:,0:3].squeeze(dim=0).cpu().numpy().tolist(),
                "debris_position": self._obj_pos.squeeze(dim=0).cpu().numpy().tolist(),
                "target_position": self.target_position.squeeze(dim=0).cpu().numpy().tolist(),
                "contact_force": ee_efforts_obs.cpu().numpy().tolist(),
                "command_force": self.F_command.squeeze(dim=0).cpu().numpy().tolist(),
            }
        ...
        #print("pos command 2: " + str(obj_pos[:,0:2].to(self.device) - pos_command_[:,0:2]))
        #print("obj pos : " + str(obj_pos[:,0:2].to(self.device)))

        #pos_command_xy = ee_pose[:,0:2] #
        pos_command_xy =self._obj_pos[:,0:2].to(self.device) - pos_command_[:, 0:2]
        pos_command_z = z_command 
        #print("pos command : " + str(pos_command))
        #pos_command = pos_command.squeeze(dim = 0)
        #print("pos command : " + str(pos_command))
        pos_command = th.cat([pos_command_xy.squeeze(dim=0), pos_command_z.squeeze(dim=0)]).to(self.device)

        #print("pos command cartesian: " + str(pos_command))
        ...
        #print("ang command: " + str(self.ang_command))

        #print("pos command filtered: " + str(pos_command))
        ...
        #joints_command[:] = th.remainder((joints_command[:] + _pi), _2_pi) - _pi
        #print("joint commands : " + str(joints_command))
    ...
    def compute_rewards(self)-> th.Tensor:
        ...
        #dist_a = - self.dist_normalized(0, self.init_part_target_max_dist  , self.part_target_max_dist)
        ...
        #print("dist target obj: " + str(dist_b))
        reward[0] =  dist_c + dist_b
        #print("reward: ")
        ...
        self.total_reward = self.total_reward + reward[0].item()
    ...
    def reset_rigid_object_position(self, prim, ee_pos, target_pos):
        if self.cfg["task"]["init"]["randomize"] == True:
            ...
            self.prev_obj_pos = medium_pos[:,0:3]
            self.prev_obj_pos[:,2] = self.cfg["task"]["action"]["z_command"]

            obj_ori_z = np.random.uniform(low = 0, high = 2 * np.pi)
        else:
            medium_pos = th.tensor([[self.cfg["task"]["init"]["medium"][0], self.cfg["task"]["init"]["medium"][1], self.cfg["task"]["init"]["medium"][2]]], device = self.device)
            obj_ori_z = 0
            self.prev_obj_pos = medium_pos[:,0:3]
            self.prev_obj_pos[:,2] = self.cfg["task"]["action"]["z_command"]
